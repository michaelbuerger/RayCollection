public class Engine {
    ...
    public static ColorDepthPoint RaycastPointSpecTriangle(Triangle tri, int x, int y, Vec3 sensorPosition, double sensorWidth, Vec3 sensorDirection, int resolution, int backgroundColor, double wiggle, Vec3 lightPosition, double luminosity, double outrageousRange) {
        ...
        if (hitResult.hit) { // did hit, assign color + depth
            double brightness = Engine.CalcBrightnessPoint(hitResult, lightPosition, luminosity);
    ...
    public static Vec3 CalcPlaneLineIntersect(Vec3 planePoint, Vec3 planeNormal, Vec3 linePoint, Vec3 lineVec) {
        Vec3 c, n, x0, v, w;
        c = planePoint;
    ...
    public static boolean PointWithinTriangle(Triangle triangle, Vec3 point, double wiggle) {
    ...
    /* Encode integer with RGB color data, expects 0-255, doesn't filter */
    ...
    public static double CalcBrightnessDirectional(HitResult hitResult, Vec3 lightDir) {
        return Math.abs(Math.pow(lightDir.Normalized().Dot(hitResult.hitNormal.Normalized().Negative()), 1));
    }

    public static double CalcBrightnessPoint(HitResult hitResult, Vec3 lightPosition, double luminosity) {
